...
def IsQrColorRateX(image, flag):
    ...
    if maxIdx < 2 or maxIdx > len(count)-3:
        return False
    rate = np.concatenate(
        [count[maxIdx-2:maxIdx], count[maxIdx+1:maxIdx+3]], axis=0).astype(np.float32)/maxCount
    rate = IsQrRate(rate)
...
def FindQr(imgSrc):
    ...
    imgErosion = cv2.erode(imgBlur, kernel)

    # 二值化
    _, imgBinary = cv2.threshold(
        imgErosion, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)

    # 查找轮廓
    _, contours, hierarchy = cv2.findContours(
        imgBinary, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    imgSrcClone = np.copy(imgSrc)
    for cont in contours:
        x, y, w, h = cv2.boundingRect(cont)
        imgSrcClone = cv2.rectangle(
            imgSrcClone, (x, y), (x+w, y+h), color=(0, 0, 255), thickness=2)
            ...
            for k in range(len(qrPoint)):
                ...
                if ratio > 0.6 and ratio < 1.6 and ratio1 > 0.85 and ratio1 < 1.15:
                    ...
                    contour = np.concatenate(
                        [qrPoint[i], qrPoint[j], qrPoint[k]], axis=0)
                    rotatedRect = cv2.minAreaRect(contour)
...
if __name__ == '__main__':
    # 读入图像
    image = cv2.imread('/home/pi/Desktop/img.jpg')
    # 处理图像
    qrcode = FindQr(image)
